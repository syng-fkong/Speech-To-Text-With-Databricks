...
2. **Lakebase Sync** — **DEFERRED** (same root cause as Phase 0.1).
   - The bundle resource `synced_database_tables` and Terraform provider `databricks_database_synced_database_table` only support the **provisioned** Lakebase variety. Our Lakebase is **autoscale** (`projects/speech-to-text/branches/production/...` model) — confirmed when Terraform passed schema validation but the API returned "Database instance is not found" for every instance-name format tried (`speech-to-text`, `projects/speech-to-text`, `projects/speech-to-text/branches/production`).
   - The autoscale path is `databricks postgres create-synced-table` / `POST /api/2.0/postgres/synced_tables` — but that API has the same JSON-body discoverability problem as `create-catalog` (Phase 0.1): error messages say "Field 'synced_table' is required" no matter what subfields you send.
   - Path forward: same as Phase 0.1 — wait for clearer docs / CLI updates, or get the working JSON body from a Databricks contact.
   - Until Phase 2 produces real rows, the verdict app's end-to-end queue→verdict flow can be exercised by hand-inserting test rows in Postgres `app.review_queue` via psql (blocked by current sandbox classifier, but the user can run it directly).
   - Path/call_id naming decision: resolved in Phase 3 — Postgres column is `path` (matches upstream).
