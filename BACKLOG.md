...
2. **Lakebase Sync** — **MOSTLY DONE** (2026-05-14). Sync is registered, snapshot ran, 1 row materialised in Postgres `app.review_queue` (verified via UC federation `SELECT FROM lakebase_stt.app.review_queue`). Working API body shape: `POST /api/2.0/postgres/synced_tables?synced_table_id=lakebase_stt.app.review_queue` with `{"spec": {"source_table_full_name": "...", "branch": "...", "postgres_database": "databricks_postgres", "primary_key_columns": ["path"], "scheduling_policy": "SNAPSHOT", "create_database_objects_if_missing": true}}`.
   ...
   - **Follow-up needed (Phase 2.1)**: the sync-owned `app.review_queue` only has the gold-view columns (`path`, `transcript`, both NLP outputs, similarity scores, `disagreement_flags`, `_ingested_date`, `_ingested_at`). My app code expected `status`, `claimed_by`, `claimed_at` columns on the same table. Need to split: keep `review_queue` sync-owned and read-only, add `app.review_state(path PK, status, claimed_by, claimed_at)` app-owned for workflow state. Queue list/claim/release/verdict queries then LEFT JOIN `review_state` and COALESCE(status, 'pending'). Also: app SP needs SELECT grant on the sync-owned `review_queue` (the second app endpoint hit "permission denied for table review_queue").
